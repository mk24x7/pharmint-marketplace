...
export default async function ApprovalCard({
  ...
  return (
    <Container className="bg-background-secondary border border-pharmint-border flex small:flex-row flex-col p-4 rounded-md small:justify-between small:items-center gap-y-2 items-start">
          ...
          {cart.items?.slice(0, 3).map((i, index) => {
            ...
            return (
              <div
                key={i.id}
                className={clx(
                  "block w-7 h-7 border border-pharmint-border bg-background-secondary p-2 bg-cover bg-center rounded-md ml-[-5px]",
                  {
