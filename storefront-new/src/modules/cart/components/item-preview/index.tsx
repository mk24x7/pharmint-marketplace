  ...
  return (
    ...
    >
      <div className="flex gap-x-4 items-start">
        <LocalizedClientLink href={`/products/${handle}`}>
          <Thumbnail
            ...
            size="square"
            className="bg-background-secondary rounded-lg w-10 h-10"
          />
        </LocalizedClientLink>
          ...
          <div className="flex small:flex-row flex-col gap-2">
            {(item.metadata?.note as string) && (
              <div className="flex gap-x-1">
                ...
                <span className="text-xs text-pharmint-muted italic truncate max-w-44 pr-px">
                  {item.metadata?.note as string}
                </span>
              </div>
            )}
      ...
      <div className="flex flex-col-reverse items-end justify-between">
        ...
        />
        <span className="self-end text-xs text-pharmint-muted italic">
          {item.quantity}x
        </span>
      </div>
    </Container>
