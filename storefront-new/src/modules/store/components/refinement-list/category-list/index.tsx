...
const CategoryList = ({
        ...
        <div className={`flex items-center gap-2 mb-2 pl-${paddingLeft}`}>
          {hasChildren ? (
            ...
            <LocalizedClientLink
              ...
              className="flex gap-2 items-center hover:text-accent text-start hover:cursor-pointer transition-colors duration-200"
            >
  ...
  return (
    <Container className="flex flex-col p-0 divide-y divide-pharmint-border bg-background-secondary">
      <div className="flex justify-between items-center p-3">
        <Text className="text-sm font-medium text-white">Categories</Text>
        {pathname.includes("/categories") && (
          <LocalizedClientLink
            href="/store"
            className="text-xs text-pharmint-muted hover:text-accent transition-colors duration-200"
          >
            Clear
          </LocalizedClientLink>
        )}
      </div>
      <ul className="flex flex-col gap-3 text-sm p-3 text-pharmint-muted">
        {categories
